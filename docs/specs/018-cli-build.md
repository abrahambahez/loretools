...
The `lore` CLI (completed in spec 012-cli) currently requires Python, uv, and a virtual environment at the user's site. The goal is to package it as standalone executables for distribution alongside Claude Desktop skills.
...
- Top-level binary: `lore` (macOS/Linux) or `scht.exe` (Windows)
...
- Version string stamped from `pyproject.toml` into the binary, exposed via `lore --version`
...
- when a researcher runs `install.sh` (macOS/Linux) or `install.ps1` (Windows), the system must download the correct platform zip, place the binary on PATH, and create an initial `~/.config/loretools/config.json` via interactive prompts
- when a researcher on macOS downloads and unzips `scht-<version>-macos-arm64.zip`, the system must contain a top-level `lore` binary executable
- when a researcher on Linux downloads and unzips `scht-<version>-linux-x86_64.zip`, the system must contain a top-level `lore` binary executable
...
- when the researcher runs `lore --version` (without Python installed), the system must exit 0 and print the correct release version
- when the researcher runs `lore refs list`, the system must execute successfully and return library references in JSON format
...
- when `lore --version` is called, the system must report the version string from `pyproject.toml` (not a stale hardcoded value)
...
- [x] task-02: add version stamping to the CLI binary (blocks: task-01)
  ...
  - Verify `lore --version` reports the correct version from the bundle
...
- [x] task-04: create standalone install scripts as separate release assets (blocks: task-03)
  - `install.sh`: downloads correct platform zip from GitHub releases, extracts to `~/.local/bin`, patches shell rc files for PATH, creates `~/.config/loretools/config.json` via interactive prompts (email, library path, sources)
  - `install.ps1`: same flow for Windows — installs to `%LOCALAPPDATA%\Programs\scht`, persists PATH via `[Environment]::SetEnvironmentVariable`, writes config to `%USERPROFILE%\.config\loretools\config.json`
...
- [x] task-05: smoke test the release pipeline (blocks: task-04)
  ...
  - Download and unzip each bundle; run `./lore --version` and `./lore refs list`
