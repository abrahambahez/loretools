...
def test_files_get_dispatches(capsys):
    with patch("loretools.get_file", return_value="/files/smith2020.pdf") as mock_fn:
...
def test_files_get_none_result(capsys):
    with patch("loretools.get_file", return_value=None):
...
def test_files_list_plain(capsys):
    ...
    result = FilesListResult(files=[row], total=1, page=1)
    with patch("loretools.list_files", return_value=result):
        with pytest.raises(SystemExit) as exc_info:
