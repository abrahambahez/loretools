...
description: scholartools collection manager — first-time setup in Claude Co-Work, binary installation, config creation, and session-start verification. Use this when the user wants to set up scholartools for the first time, install the binary into a collection directory, create or update config.json, verify that a collection is operational, or when any scholartools command fails because the binary or config is missing.
---

A **collection** is a directory that contains `scht`, `.scholartools/config.json`, and the researcher's library data. `scht` always operates relative to the current working directory (CWD) — no global config, no PATH installation needed.
...
Run `scht` once from the collection directory to auto-create `.scholartools/config.json` with defaults:

```bash
./scht refs list
```

This creates `.scholartools/config.json` with `library_dir` set to the collection directory (CWD). No further path configuration is needed unless the user wants a non-default layout.

**Step 3 — Apply user preferences**

If the user provides API keys or wants to customize config, edit `.scholartools/config.json`. Common fields:
...
If `scht` is not found, the binary was not uploaded or the working directory is wrong. Ask the user to confirm the folder is mounted and `scht` is present.
